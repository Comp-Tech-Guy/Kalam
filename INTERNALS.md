...
| Secret | Value |
|--------|-------|
| `TAURI_SIGNING_PRIVATE_KEY` | Literal content of `kalam.key` — a single base64 line, copied as-is. **Do NOT base64-encode it again.** |
...
3. **Rust dependency cache** (`Swatinem/rust-cache@v2`) — cuts compile from ~20 min to ~2-3 min
4. **Single `tauri-action` step** — builds, creates draft GitHub Release, uploads MSI + `.sig`, generates `latest.json`
...
**Problem:** `TAURI_SIGNING_PRIVATE_KEY` must be the **literal single-line content** of the `.key` file. Re-encoding it (e.g. `[Convert]::ToBase64String`) or mangling it makes Tauri fail with `Missing encoded key in secret key`.

**Fix:** Store the raw content of `kalam.key` in the secret and pass it straight to the build. A pre-flight step (`scripts/verify-signing-keys.ps1`) signs a throwaway file with the secret and compares the resulting keynum to `plugins.updater.pubkey`, failing fast before the long build instead of mid-release.
...
- **Keys/password lost**: A lost private key or password permanently breaks updates for all existing installs. Keep backups of `kalam.key` and its password; run `scripts/verify-signing-keys.ps1` before every release.
- **Private repo**: The endpoint URL is unauthenticated — make repo public or host `latest.json` elsewhere
- **Wrong env var names**: Tauri v2 reads only `TAURI_SIGNING_PRIVATE_KEY` and `TAURI_SIGNING_PRIVATE_KEY_PASSWORD`. v1 names (`TAURI_PRIVATE_KEY`, `TAURI_KEY_PASSWORD`, `TAURI_PRIVATE_KEY_PATH`) are not used during `tauri build`.
- **Keys/password lost**: A lost private key or password permanently breaks updates for all existing installs. Keep backups of `kalam.key` and its password; run `scripts/verify-signing-keys.ps1` before every release.
